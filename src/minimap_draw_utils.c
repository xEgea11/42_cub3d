...
void put_pixel_minimap(t_game *game, int x, int y)           //<---- It needs to be refactored, i'm still working on it
{
    ...
    while (i < (int)game->y_scale_minimap)
    {
        ...
        while (j < (int)game->x_scale_minimap)
        {
            ...
            if (draw_x >= 0 && draw_x < MINIMAP_WIDTH && draw_y >= 0 && draw_y < MINIMAP_HEIGHT) 
            {
                if (game->data->map2d_square[y][x] == EMPTY || game->data->map2d_square[y][x] == 'N')
                    mlx_put_pixel(game->img, draw_x, draw_y, FLOOR_COLOR);
                else if (game->data->map2d_square[y][x] == WALL || game->data->map2d_square[y][x] == OUT_OF_BOUNDS)
...
void draw_square_minimap(t_game *game, int player_x, int player_y)
{
    ...
    while (i <= 2)
    {
        ...
        while (j <= 2)
        {
            if (player_x + i >= 0 && player_x + i < MINIMAP_WIDTH && player_y + j >= 0 && player_y + j < MINIMAP_HEIGHT)
